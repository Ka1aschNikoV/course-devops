...
const SERVER_URL = 'http://host.docker.internal:8197/state';  // Keep the /state endpoint
...
describe('Server Put State Tests', () => {
    ...
    it('should respond if user is logged in', async () => {
        const response = await axios.get("http://host.docker.internal:8198/controlpanel.html", {
            auth: {
                username: 'user1',
                password: 'your_mom',
            }
    ...
    it('should be resetted when RUNNING -> INIT', async () => {
        ...
        const responseLogs = await axios.get("http://host.docker.internal:8197/run-log")
